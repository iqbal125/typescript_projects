...
const TodosTablePage: React.FC = () => {
    ...
    const columns = [
        ...
        columnHelper.accessor('description', {
            header: 'Description',
            cell: (info) => <div className='max-w-2xl truncate'>{info.getValue() ?? "--"}</div>,
        }),
    ...
    const showEmptyRow = !isLoading && !hasRows;
    const showDataRows = !isLoading && hasRows;
    const todosSummary = meta
        ? `Showing ${offset + 1}–${Math.min(offset + limit, meta.total)} of ${meta.total} todo(s)`
        : 'Loading...';
    const pageLabel = `Page ${page + 1} of ${meta?.totalPages ?? '—'}`;
    const isPrevDisabled = page === 0 || isLoading;
    const isNextDisabled = !meta || page + 1 >= meta.totalPages || isLoading;
    ...
    return (
        <div className="space-y-4 w-full mx-auto">
            ...
            <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">{todosSummary}</p>
