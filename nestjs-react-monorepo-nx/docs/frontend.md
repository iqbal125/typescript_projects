...
The frontend is a React + Vite app located in apps/frontend/.
...
API client is configured in apps/frontend/src/api/.

Typical setup:
- Axios base client uses `VITE_API_URL`.
...
Frontend dev server host/port also come from root env values:
- `VITE_FRONTEND_HOST`
- `VITE_FRONTEND_PORT`
...
- @org/shared-utils for shared helper functions.
